<div class="col-12">
    <form id="filterForm" method="GET" class="row g-3 mb-3">
        <div class="col-md-4">
            <input type="text" name="question" class="form-control form-control-sm" placeholder="🔍 Search question..." value="{{ old('question', $filters['question'] ?? '') }}">
        </div>
        <div class="col-md-3">
            <select name="display_on" class="form-select form-select-sm">
                ...
                <option value="checkout" {{ old('display_on', $filters['display_on'] ?? '') == 'checkout' ? 'selected' : '' }}>🛒 Checkout</option>
            </select>
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-2">
            <a href="{{ route('admin.faq.index') }}" class="btn btn-primary w-100 text-white btn-sm"><i class="bi bi-x-circle"></i></a>
        </div>
    </form>
</div>